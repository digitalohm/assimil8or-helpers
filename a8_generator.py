...
parser.add_argument('-p', '--preset', required=True, help='This is the name of the preset')
parser.add_argument('-n', '--number', required=True, help='This is the number of the preset, which is set inside the yml')
...
for root, dirnames, filenames in os.walk(sample_dir):
    for file in filenames:
        (shortname, extension) = os.path.splitext(file)
        if extension == '.wav' and os.path.join(root,file) != sample_dir and not os.path.exists(os.path.join(dest_dir, file)):
